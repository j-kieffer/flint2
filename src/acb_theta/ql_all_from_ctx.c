...
static int
acb_theta_ql_all_with_t(acb_ptr th, const acb_theta_ql_ctx_t ctx, slong guard, slong prec)
{
	...
	if (ctx->z_is_zero)
	{
		res = acb_theta_ql_naive_worker_gen(rts, acb_theta_ql_ctx_exp_zs(ctx) + (ctx->t_is_zero ? 0 : g),
			1, acb_theta_ql_ctx_exp_tau(ctx), acb_theta_ql_ctx_vs(ctx),
			acb_theta_ql_ctx_dists(ctx), acb_theta_ql_ctx_cho(ctx), 1, prec);
	}
	else
	{
		res = acb_theta_ql_naive_worker_gen(rts, acb_theta_ql_ctx_exp_zs(ctx) + (ctx->t_is_zero ? g : 4 * g),
			1, acb_theta_ql_ctx_exp_tau(ctx), acb_theta_ql_ctx_vs(ctx) + g,
			acb_theta_ql_ctx_dists(ctx) + n, acb_theta_ql_ctx_cho(ctx), 1, prec);
	}
	for (j = 0; (j < n * n) && res; j++)
	{
		/* Ignore odd theta constants */
		if (ctx->z_is_zero && ctx->t_is_zero && !acb_theta_char_is_even(j, g))
		{
			continue;
		}
	...
	{
		acb_theta_char_get_a(new_v, a, g);
		_arb_vec_neg(new_v, new_v, g);
		_arb_vec_add(new_v, new_v, acb_theta_ql_ctx_vs(ctx) + g, g, prec);
		new_prec = prec + acb_theta_dist_addprec(acb_theta_ql_a0_dists(ctx)
			+ (ctx->z_is_zero ? 0 : n) + a);
		acb_theta_naive_radius(R2, eps, acb_theta_ql_ctx_cho(ctx), 0, new_prec);
